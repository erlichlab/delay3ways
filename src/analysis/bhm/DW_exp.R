...
# Describe the model
dw_model = bf(choice ~ inv_logit((rewmag / (1 + exp(logk) * delay) - smag) / noise),
...
save.image(file='../../../data/dw_fit.RData')

# Perform leave-one-out cross validation. 
loo = brms::LOO(dw_fit, nsamples = 4000, cores=10)
save.image(file='../../../data/dw_fit.RData')

# Perform k-fold cross validation
kfold <- brms::kfold(dw_fit, K=10, update_args=c(chains = 10, cores = 10))
save.image(file='../../../data/dw_fit.RData')
